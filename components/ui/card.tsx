...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "bg-card text-card-foreground flex flex-col gap-5 rounded-lg border border-slate-200 py-5 shadow-xs dark:border-neutral-800",
        className
      )}
      {...props}
    />
...
function CardTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("text-sm font-semibold leading-none text-slate-950 dark:text-white", className)}
      {...props}
    />
  )
}
